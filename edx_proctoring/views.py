...
class BaseReviewCallback(object):
    ...
    def make_review(self, attempt, data, backend=None):
        ...
        if review.reviewed_by is not None and not is_user_course_or_global_staff(review.reviewed_by, course_id):
            LOG.warning(
                'User {user} does not have the required permissions to submit a review for attempt_code {attempt_code}.'
                .format(user=review.reviewed_by, attempt_code=attempt_code)
            )
